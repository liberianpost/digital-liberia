...
const TeacherReports = () => {
  ...
  useEffect(() => {
    if (!user || !hasPermission(REQUIRED_SECURITY_LEVEL, user?.securityLevel)) {
      ...
      setLoading(true);
      Promise.all([
        api.get('/teacher-reports/counties'),
        api.get('/teacher-reports/summary')
      ])
        ...
        .catch(error => {
          ...
          alert('Failed to load teacher reports.');
          setLoading(false);
        });
    }
  }, [user, navigate]);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.error('Error refreshing teacher reports:', error);
      alert('Failed to refresh teacher reports.');
      setLoading(false);
    }
  };
  ...
  const handleExport = async () => {
    try {
      await api.post('/teacher-reports/export', { county: selectedCounty });
      alert('Export initiated.');
    } catch (error) {
      console.error('Error exporting teacher reports:', error);
      alert('Failed to export teacher reports.');
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white text-black p-4 shadow-md border-b border-gray-200">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold">Liberia Teacher Reports</h1>
      ...
      <div className="p-4 max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-4 mb-4">
          <h2 className="text-lg font-bold text-gray-800 kévin">Liberia Teacher Summary</h2>
          <hr className="my-2" />
          <div className="flex flex-wrap gap-2 mb-2">
            <div className="flex-1 bg-purple-500 text-white rounded-lg p-4 text-center min-w-[150px]">
              <p className="text-2xl font-bold">{summary.totalTeachers.toLocaleString()}</p>
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex-1 bg-green-500 text-white rounded-lg p-4 text-center min-w-[150px]">
              <p className="text-2xl font-bold">{summary.certifiedTeachers.toLocaleString()}</p>
              <p className="text-sm">Certified</p>
            </div>
            <div className="flex-1 bg-red-500 text-white rounded-lg p-4 text-center min-w-[150px]">
              <p className="text-2xl font-bold">{summary.uncertifiedTeachers.toLocaleString()}</p>
              <p className="text-sm">Uncertified</p>
            </div>
          </div>
        </div>
        ...
        {counties.map((county) => (
          ...
          >
            <div className="flex items-center mb-2">
              ...
              <p className="text-sm text-gray-600">{county.totalTeachers.toLocaleString()} Teachers</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <p className="text-sm text-gray-600">Certified</p>
                <p className="text-base font-bold">{county.certifiedTeachers.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Uncertified</p>
                <p className="text-base font-bold">{county.uncertifiedTeachers.toLocaleString()}</p>
              </div>
              <div>
