...
const MoaDashboard = () => {
  ...
  useEffect(() => {
    const checkAuthAndFetchData = async () => {
      ...
      const dssn = localStorage.getItem("MOA_DSSN") || "";
      ...
      try {
        ...
        // Mock analytics data
        const mockAnalytics = {
          registeredFarmers: 125000,
          farmingPermits: 87500,
          agriculturalExports: 425000000,
          foodSecurityIndex: "72%",
          cropTypes: [
            { type: "Rice", farmers: 65000, percentage: 52 },
            { type: "Cassava", farmers: 35000, percentage: 28 },
            { type: "Coffee", farmers: 15000, percentage: 12 },
            { type: "Cocoa", farmers: 8000, percentage: 6 },
            { type: "Rubber", farmers: 2000, percentage: 2 }
          ]
        };
  ...
  const handleLogout = () => {
    const keys = Object.keys(localStorage).filter(key => key.startsWith('MOA_'));
    ...
    <div className="relative min-h-screen w-full bg-white text-gray-800 font-inter overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 w-full z-50 bg-white shadow-lg border-b border-gray-200">
        <div className="flex flex-col px-6 py-4 max-w-7xl mx-auto">
          ...
          {/* Ministry of Agriculture Logo and Title - Bottom Row */}
          <div className="flex items-center justify-center space-x-4">
            <img src="/logos/moa.png" alt="MOA Logo" className="w-12 h-12 object-contain" />
            <div className="text-center">
      ...
      <main className="relative z-30 pt-40 pb-20 px-6">
                ...
                <div className="flex items-center space-x-6">
                  <div className="relative">
                    <div className="w-24 h-24 bg-gradient-to-br from-green-500 to-green-700 rounded-2xl p-1 shadow-xl">
                      <img
                        src={userProfile?.image || "/logos/moa-user.png"}
                        alt="Profile"
                        className="w-full h-full object-cover rounded-2xl"
                      />
                    </div>
                    <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-green-500 rounded-full border-4 border-white flex itemsCenter justify-center">
        ...
        <section className="max-w-7xl mx-auto mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white border border-green-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-green-800">{formatNumber(analytics.registeredFarmers)}</h3>
                  <p className="text-green-600">Registered Farmers</p>
                </div>
            ...
            <div className="bg-white border border-blue-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-blue-800">{formatNumber(analytics.farmingPermits)}</h3>
                  <p className="text-blue-600">Farming Permits</p>
                </div>
            ...
            <div className="bg-white border border-purple-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-purple-800">{formatCurrency(analytics.agriculturalExports)}</h3>
                  <p className="text-purple-600">Agricultural Exports</p>
                </div>
            ...
            <div className="bg-white border border-orange-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-orange-800">{analytics.foodSecurityIndex}</h3>
                  <p className="text-orange-600">Food Security Index</p>
                </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-md">
            <h3 className="text-xl font-bold text-gray-800 mb-6">Crop Types Distribution</h3>
            <div className="space-y-4">
              {analytics.cropTypes.map((cropType, index) => (
                <div key={cropType.type} className="flex items-center justify-between">
                  <span className="text-gray-700 w-32 truncate font-medium">{cropType.type}</span>
                  <div className="flex-1 mx-4">
                    <div className="h-3 bg-gray-200 rounded-full">
                      <div 
                        className="h-full bg-gradient-to-r from-green-400 to-green-600 rounded-full"
                        style={{ width: `${cropType.percentage}%` }}
                      ></div>
                    </div>
                  </div>
                  <span className="text-green-800 font-semibold w-20 text-right">
                    {formatNumber(cropType.farmers)}
                  </span>
                </div>
              ))}
            </div>
        ...
        <section className="max-w-7xl mx-auto mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: "👨‍🌾", label: "Farmer Registration", color: "from-green-600 to-green-700" },
              { icon: "📋", label: "Permit Management", color: "from-blue-600 to-blue-700" },
              { icon: "🌾", label: "Crop Monitoring", color: "from-purple-600 to-purple-700" },
              { icon: "📦", label: "Export Services", color: "from-orange-600 to-orange-700" },
              { icon: "🚜", label: "Equipment Services", color: "from-red-600 to-red-700" },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { 
                ...
                description: "Access to agriculture data, policy management, and system administration"
              },
              { 
                icon: "👨‍🌾", 
                label: "Farmer Services", 
                color: "from-blue-600 to-blue-700",
                description: "Register farmers, provide support services, and manage farmer data"
              },
              { 
                icon: "📋", 
                label: "Permit Officers", 
                color: "from-purple-600 to-purple-700",
                description: "Process farming permits, land use approvals, and agricultural licenses"
              },
              { 
                icon: "🌾", 
                label: "Agricultural Officers", 
                color: "from-orange-600 to-orange-700",
                description: "Monitor crops, provide technical support, and implement agricultural programs"
              },
              { 
                icon: "📦", 
                label: "Export Officers", 
                color: "from-red-600 to-red-700",
                description: "Manage export processes, quality control, and international trade compliance"
              },
              { 
                icon: "🔐", 
      ...
      <footer className="relative z-30 py-8 text-center bg-gray-50 border-t border-gray-200">
        <div className="max-w-7xl mx-auto">
          <p className="text-gray-600 text-sm">
            © {new Date().getFullYear()} Ministry of Agriculture - Digital Liberia. All rights reserved.
          </p>
          <p className="text-gray-500 text-xs mt-2">
            Advanced Agriculture Management System • Powered by Digital Liberia NDMS
          </p>
        </div>
      </footer>
    </div>
