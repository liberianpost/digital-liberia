...
const MocDashboard = () => {
  ...
  useEffect(() => {
    const checkAuthAndFetchData = async () => {
      ...
      const dssn = localStorage.getItem("MOC_DSSN") || "";
      ...
      try {
        ...
        // Mock analytics data
        const mockAnalytics = {
          registeredBusinesses: 125000,
          tradeLicenses: 87500,
          exportCertificates: 32500,
          importPermits: 28500,
          businessTypes: [
            { type: "Retail", count: 65000, percentage: 52 },
            { type: "Services", count: 35000, percentage: 28 },
            { type: "Manufacturing", count: 15000, percentage: 12 },
            { type: "Agriculture", count: 8000, percentage: 6 },
            { type: "Other", count: 2000, percentage: 2 }
          ]
        };
  ...
  const handleLogout = () => {
    const keys = Object.keys(localStorage).filter(key => key.startsWith('MOC_'));
    ...
    <div className="relative min-h-screen w-full bg-white text-gray-800 font-inter overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 w-full z-50 bg-white shadow-lg border-b border-gray-200">
        <div className="flex flex-col px-6 py-4 max-w-7xl mx-auto">
          ...
          {/* Ministry of Commerce Logo and Title - Bottom Row */}
          <div className="flex items-center justify-center space-x-4">
            <img src="/logos/moc.png" alt="MOC Logo" className="w-12 h-12 object-contain" />
            <div className="text-center">
      ...
      <main className="relative z-30 pt-40 pb-20 px-6">
                ...
                <div className="flex items-center space-x-6">
                  <div className="relative">
                    <div className="w-24 h-24 bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl p-1 shadow-xl">
                      <img
                        src={userProfile?.image || "/logos/moc-user.png"}
                        alt="Profile"
        ...
        <section className="max-w-7xl mx-auto mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white border border-blue-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-blue-800">{formatNumber(analytics.registeredBusinesses)}</h3>
                  <p className="text-blue-600">Registered Businesses</p>
                </div>
            ...
            <div className="bg-white border border-green-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-green-800">{formatNumber(analytics.tradeLicenses)}</h3>
                  <p className="text-green-600">Trade Licenses</p>
                </div>
            ...
            <div className="bg-white border border-purple-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-purple-800">{formatNumber(analytics.exportCertificates)}</h3>
                  <p className="text-purple-600">Export Certificates</p>
                </div>
            ...
            <div className="bg-white border border-orange-200 rounded-2xl p-6 shadow-md hover:scale-105 transition-transform duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-bold text-orange-800">{formatNumber(analytics.importPermits)}</h3>
                  <p className="text-orange-600">Import Permits</p>
                </div>
              ...
              <div className="mt-4 h-2 bg-orange-100 rounded-full">
                <div className="h-full bg-orange-500 rounded-full w-1/3"></div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-md">
            <h3 className="text-xl font-bold text-gray-800 mb-6">Business Types Distribution</h3>
            <div className="space-y-4">
              {analytics.businessTypes.map((businessType, index) => (
                <div key={businessType.type} className="flex items-center justify-between">
                  <span className="text-gray-700 w-32 truncate font-medium">{businessType.type}</span>
                  <div className="flex-1 mx-4">
                    <div className="h-3 bg-gray-200 rounded-full">
                      <div 
                        className="h-full bg-gradient-to-r from-blue-400 to-blue-600 rounded-full"
                        style={{ width: `${businessType.percentage}%` }}
                      ></div>
                    </div>
                  </div>
                  <span className="text-blue-800 font-semibold w-20 text-right">
                    {formatNumber(businessType.count)}
                  </span>
                </div>
              ))}
            </div>
        ...
        <section className="max-w-7xl mx-auto mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: "🏢", label: "Business Registration", color: "from-blue-600 to-blue-700" },
              { icon: "📋", label: "Trade Licenses", color: "from-green-600 to-green-700" },
              { icon: "📦", label: "Export Services", color: "from-purple-600 to-purple-700" },
              { icon: "📦", label: "Import Services", color: "from-orange-600 to-orange-700" },
              { icon: "🤝", label: "Trade Agreements", color: "from-red-600 to-red-700" },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { 
                ...
                description: "Access to commerce data, policy management, and system administration"
              },
              { 
                icon: "🏢", 
                label: "Business Registration", 
                color: "from-green-600 to-green-700",
                description: "Process business registrations, verifications, and compliance checks"
              },
              { 
                icon: "📋", 
                label: "License Officers", 
                color: "from-purple-600 to-purple-700",
                description: "Process trade licenses, conduct inspections, and ensure compliance"
              },
              { 
                icon: "📦", 
                label: "Trade Officers", 
                color: "from-orange-600 to-orange-700",
                description: "Manage import/export processes, certifications, and trade compliance"
              },
              { 
                icon: "🤝", 
                label: "Trade Relations", 
                color: "from-red-600 to-red-700",
                description: "Manage trade agreements, international relations, and economic partnerships"
              },
              { 
                icon: "🔐", 
      ...
      <footer className="relative z-30 py-8 text-center bg-gray-50 border-t border-gray-200">
        <div className="max-w-7xl mx-auto">
          <p className="text-gray-600 text-sm">
            © {new Date().getFullYear()} Ministry of Commerce - Digital Liberia. All rights reserved.
          </p>
          <p className="text-gray-500 text-xs mt-2">
            Advanced Commerce Management System • Powered by Digital Liberia NDMS
          </p>
        </div>
      </footer>
    </div>
